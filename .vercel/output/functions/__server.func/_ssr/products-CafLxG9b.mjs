import { c as createServerRpc, a as connectToDatabase } from "./mongodb-HGuSOqhb.mjs";
import { c as createServerFn } from "./server-CHo3IP9P.mjs";
import { l as libExports } from "../_libs/mongodb.mjs";
import { p as products } from "./products-data-LRMOsjPP.mjs";
...
async function ensureProductsSeeded(db) {
  ...
  if (count < products.length) {
    if (count > 0) {
      console.log("New products detected, dropping database collection to re-seed...");
      await db.collection("products").deleteMany({});
    }
    const listToInsert = products.map((p) => {
      const details = detailedInfoMap[p.id] || {
        ...
        specs: [{
          ...
          label: "Price",
          value: `PKR ${p.price.toLocaleString()}`
        }, {
          label: "Status",
          value: p.inStock ? "In Stock" : "Out of Stock"
        }]
      };
      return {
        _id: isValidObjectId(p.id) ? new libExports.ObjectId(p.id) : new libExports.ObjectId(),
        customId: p.id,
        name: p.name,
        brand: p.brand,
        cat: p.cat,
...
const getProductByIdFn = createServerFn({
  ...
  try {
    ...
    const staticProduct = products.find((p) => p.id === data.id);
    if (staticProduct) {
