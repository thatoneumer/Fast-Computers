  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        {/* Story Section */}
        <section className="mx-auto max-w-7xl px-6 py-20 grid lg:grid-cols-2 gap-16 items-center" style={{ perspective: "1200px" }}>
          {/* Text side — slides in from left with 3D rotate */}
          <motion.div
          ...
          {/* Image side — slides in from right with 3D rotate + Tilt on hover */}
          <motion.div
            initial={{ opacity: 0, x: 50, rotateY: -12, transformPerspective: 1200 }}
            whileInView={{ opacity: 1, x: 0, rotateY: 0 }}
            viewport={{ once: true }}
            ...
            style={{ transformStyle: "preserve-3d" }}
          >
            <Tilt3D maxTilt={8} scale={1.03} className="relative">
              <div className="absolute -inset-4 border border-primary/40" />
              <img
                src="https://images.unsplash.com/photo-1593642632823-8f785ba67e45?auto=format&fit=crop&w=900&q=80"
                alt="Fast Computers office"
                className="relative w-full aspect-[4/3] object-cover red-border-glow"
              />
        ...
        {/* Stats Section */}
        <section className="border-y border-border slash-bg" style={{ perspective: "1200px" }}>
          <motion.div
            initial={{ opacity: 0, rotateX: 10, transformPerspective: 1200 }}
            whileInView={{ opacity: 1, rotateX: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7 }}
            style={{ transformStyle: "preserve-3d" }}
            className="mx-auto max-w-7xl px-6 py-16 grid grid-cols-2 md:grid-cols-4 gap-6"
          >
            {stats.map((s, i) => (
              <motion.div
                key={s.label}
                ...
                style={{ transformStyle: "preserve-3d" }}
              >
                <Tilt3D maxTilt={10} scale={1.05} className="text-center border border-border bg-card p-8 hover:border-primary transition group h-full">
        ...
        {/* CTA Section */}
        <section className="mx-auto max-w-7xl px-6 py-24 text-center" style={{ perspective: "1000px" }}>
          <motion.h3
            initial={{ opacity: 0, y: 30, rotateX: 14, transformPerspective: 1000 }}
            whileInView={{ opacity: 1, y: 0, rotateX: 0 }}
            viewport={{ once: true }}
